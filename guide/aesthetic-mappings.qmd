...
# Explicitily drop the missing data so that when we want to show the warnings,
# we only get those are relevant.
...
There are two reasons why you may want to work around the automatic grouping.

1. _Opting In_ - To limit the scope the grouping to some of the layers.
   You do this by mapping the discrete variable to aesthetics like `color` or `fill` at the individual geom level.
...
2. _Opting Out_ - To override the grouping that is triggered by the global aesthetics.
   In most cases, you will run into this when you have groups that have one point yet you need a connecting line.
   ...
   (
       ...
       + geom_path(stat="summary", fun_y=np.mean)
   )
   ```
   
   The solution is to set the `group` in the line drawing layer to a constant.

   ```{python}
   (
       ggplot(penguins, aes(x="species", y="flipper_length_mm"))
       + geom_point(stat="summary", fun_y=np.mean)
       + geom_path(stat="summary", fun_y=np.mean, group=1)
   )
   ```

   Note that, for this particular case you should ignore the warning if the plot is what you expect it to be.
